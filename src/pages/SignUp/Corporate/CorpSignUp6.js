...
export default class CorpSignUp6 extends Component {  
  constructor(props) {
    super(props);
    ...
    try {
      ...
      if(register.password.length < 8) {
        ...
        if(response.message.password) {
          dispatch(setLoading(false))
          this.setState({error: true, message: response.message.password[0]})
        } 
          ...
          if(response.success) {
            dispatch(saveUserDetailsRedux(response.data))
            let customerType = response.data.customer_type.toLowerCase()
